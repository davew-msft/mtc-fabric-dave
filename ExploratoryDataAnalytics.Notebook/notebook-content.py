...
%md ## How I do Data Discovery and EDA (Exploratory Data Analytics)

I'll walk you through how I "learn" a new dataset.  We'll use a sample "flights" dataset.  We use notebook to get our data to "tell a story" and help the next analyst understand what we've done so far.  

We want to get to a point where we know enough about the data that we can build a rudimentary ML model against the existing data to predict if a flight will be delayed or not.  
...
%md doublecheck everything mounted correctly
...
%md ### Infer a Data Schema
If the structure of the data source is unknown, you can have Spark automatically infer the schema.

In this case, you will load data about airports without knowing the schema.  This is the most common use case...I have a new dataset and I have no idea what it contains
...
# _We could've also done the above in Scala, or more likely, SQL_.
# 
...
%md ### Determine the Presence of Duplicates
...
%md ### Explore Summary Statistics and Data Distribution
...
# * The *DayofMonth* is a value between 1 and 31, and the mean is around halfway between these values; which seems about right. 
# * The same is true for the *DayofWeek* which is a value between 1 and 7. 
# * *Carrier* is a string, so there are no numeric statistics
# * we can ignore the statistics for the airport IDs - they're just unique identifiers for the airports, not actually numeric values. 
# * The departure and arrival delays range between 63 or 94 minutes ahead of schedule, and over 1,800 minutes behind schedule. The means are much closer to zero than this, and the standard deviation is quite large; so there's quite a bit of variance in the delays. 
# * The *Late* indicator is a 1 or a 0, but the mean is very close to 0; which implies that there significantly fewer late flights than non-late flights.
# 
...
%md Yes, it looks like there are significantly more non-late flights than late ones - we can see this more clearly with a visualization, so let's use the inline **%sql** magic to query the table and bring back some results we can display as a chart:
...
%md ### Explore Relationships in the Data
...
%md In this notebook we've cleaned the flight data, and explored it to identify some potential relationships between features of the flights and their lateness.
